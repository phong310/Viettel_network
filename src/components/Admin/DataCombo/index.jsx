...
export default function DataCombo({ tab, dataList, triggerSearch }) {
    ...
    useEffect(() => {
        if (dataList.length > 0) {
            const DataSearch = dataList.filter((item) => {
                if (item.type === 'data-combo') return item
            })
            setRowData(DataSearch)
        } else {
            setRowData(dataCombo)
        }

    }, [dataCombo, dataList, triggerSearch])
        ...
        <Box sx={{ ...styleBoxContainer }}>
            <Paper sx={{ width: '100%', overflow: 'hidden', }}>
                <TableCell sx={{ ...tableCellTitle }}>
                    <Typography variant='h5'>Danh sách Combo Viettel  <Chip label={rowData.length} color="error" /></Typography>
