...
import { getALlCombo, getALlOffer, getALlSieuToc } from '../../Api/apiRequest';
...
export default function DataOffer({ tab, dataList, triggerSearch }) {
    ...
    useEffect(() => {
        if (dataList.length > 0) {
            const DataSearch = dataList.filter((item) => {
                if (item.type === 'data-offer') return item
            })
            setRowData(DataSearch)
        } else {
            setRowData(dataOffer)
        }

    }, [dataOffer, dataList, triggerSearch])
